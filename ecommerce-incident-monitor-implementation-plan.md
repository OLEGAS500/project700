...
Milestone 9.5 Merchant Center item-issue incident signals is complete and operationally verified through clean PostgreSQL CI. Complete successful item-issue checks are grouped by a configuration-scoped debounce candidate: the first affected snapshot creates a pending candidate, a second distinct complete affected snapshot confirms one `merchant_item_issues` incident, and a complete healthy snapshot dismisses an unconfirmed candidate. Partial, failed, authentication-failed, or configuration-incompatible checks do not create or dismiss business conclusions. The evaluator and recovery path take a store-scoped advisory lock, lock the item-issues source-check row, and read status plus items through the same transaction client; the lock query uses an inner join so PostgreSQL can apply `FOR UPDATE` without locking the nullable side of an outer join. Severity is `critical` when any normalized issue has critical/error-like severity and `warning` otherwise; evidence is persisted as bounded issue samples and grouped signals without raw provider payloads. Recovery uses the shared transition primitive and requires two consecutive complete healthy checks; a resolved incident is not reopened by a later occurrence. Existing incident list/detail and alert-preferences UI expose the new type, labels, severity, issue evidence, and mute control, including all six supported incident types in the mute contract. Clean CI runs `29437576438` and `29437578909` passed 242 unit tests, 37/37 PostgreSQL smoke tests, typecheck, lint, and build. Real Merchant API calls remain a separate staging gate.

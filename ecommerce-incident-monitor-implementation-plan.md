...
Milestone 9.8.1 Cross-source Offer ID mapping is complete and operationally verified through clean GitHub CI. The read-only `getCrossSourceProductMatchSummary()` facade compares explicitly selected immutable feed and Merchant item-issues snapshots for the same store under a repeatable-read transaction. Identity matching uses normalized `offer_id` first and the existing normalized `stable_key` contract only when an offer ID is unavailable; duplicate identities become `ambiguous` rather than being guessed. The result separates `matched`, `feed_only`, `merchant_only`, and `ambiguous`, returns bounded whitelisted samples and capped totals, and never reads later snapshots or provider payloads. Both source checks must be successful, the Merchant check must use the supported item-issues version, and its immutable account-derived configuration hash must still match the connected store; missing, partial, failed, cross-store, or configuration-incompatible pairs cannot produce comparable mapping output. No incident mutations, provider calls, or remediation write actions are included. Exact-head CI runs `29450503145` and `29450505082` passed 255 unit tests, 42/42 PostgreSQL smoke tests, typecheck, lint, and build, including normalization collisions, Unicode, duplicate identities, bounded samples, cross-store isolation, later-snapshot isolation, partial Merchant snapshots, and configuration fencing. Real Merchant API staging remains a separate provider gate.

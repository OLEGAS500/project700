...
Milestone 9.4.1 Merchant Center product status aggregation is code-complete pending clean PostgreSQL CI and the manual provider gate. The server-only worker calls the current Merchant API `accounts/v1/accounts/{accountId}/aggregateProductStatuses` endpoint, refreshes near-expiry OAuth access tokens through the existing fenced lease, and never exposes tokens or raw provider responses. Approved, pending, and disapproved counts are bounded, normalized from both current `stats` and documented `statistics` response shapes, and persisted into the existing snapshot columns plus redacted `source_checks.metadata_json`. Successful, partial, authentication-failed, source-unavailable, rate-limited, timeout, and malformed-response paths produce stable source-check results. Connected stores include the check in the normal snapshot flow; a standalone idempotent Merchant Center snapshot runner is also exported for scheduled execution. Unit fixtures cover provider classification and refresh behavior, while gated PostgreSQL smoke covers encrypted credentials, persistence, repeat execution, and source-check deduplication. Real Merchant API calls remain a separate staging gate; item-level issues, product imports, and product status aggregation UI remain in 9.4.2 and later slices.

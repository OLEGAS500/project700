...
Milestone 9.8.2 Feed x Merchant correlation is complete and operationally verified through clean PostgreSQL CI. It augments one already-confirmed `catalog_drop` rather than creating a second incident or changing the shared recovery state machine. The correlation reads only the candidate's immutable first drop snapshot, the frozen catalog-drop thresholds, a bounded 7-14 snapshot Merchant-approved baseline, and the immutable same-snapshot Offer ID mapping. It locks the store, observed snapshot, relevant checks, and baseline observations before evaluating. It requires a completed snapshot; successful feed, Merchant status, and complete Merchant identity checks; matching current account-derived hashes; a comparable mapping with no ambiguous identities or count-cap truncation; at least one Merchant identity absent from the feed; and a Merchant approved-product decline that independently meets the captured catalog-drop thresholds. Bounded samples remain valid evidence, while partial, failed, incomplete, legacy, configuration-mismatched, ambiguous, or count-truncated evidence produces no lifecycle mutation. A successful correlation raises the existing catalog-drop confidence, stores bounded whitelisted samples and two signals, records a deduplicated timeline event, and runs before immutable open-alert payload creation. Standard catalog-drop recovery remains feed-based and continues to require two consecutive healthy feed checks, so missing or partial Merchant evidence cannot block recovery or reopen a resolved incident. The status collector now captures the immutable Merchant account configuration hash with each aggregate-status result; PostgreSQL smoke coverage exercises confirmation, account-fenced approved counts, alert payload capture, partial/ambiguous/configuration rejection, duplicate replay, a concurrent partial replacement, and feed recovery without provider calls. Exact-head CI run `29487579408` for `b7ed00d` passed unit tests, PostgreSQL smoke, the fixture lifecycle smoke, typecheck, lint, and build. Real Merchant API staging remains a separate provider gate.

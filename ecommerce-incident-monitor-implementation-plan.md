...
Milestone 9.5 Merchant Center item-issue incident signals is implemented as a narrow backend and read-only UI slice pending live PostgreSQL CI verification. Complete successful item-issue checks are grouped by a configuration-scoped debounce candidate: the first affected snapshot creates a pending candidate, a second distinct complete affected snapshot confirms one `merchant_item_issues` incident, and a complete healthy snapshot dismisses an unconfirmed candidate. Partial, failed, authentication-failed, or configuration-incompatible checks do not create or dismiss business conclusions. Severity is `critical` when any normalized issue has critical/error-like severity and `warning` otherwise; evidence is persisted as bounded issue samples and grouped signals without raw provider payloads. Recovery uses the shared transition primitive and requires two consecutive complete healthy checks; a resolved incident is not reopened by a later occurrence. Existing incident list/detail and alert-preferences UI expose the new type, labels, severity, issue evidence, and mute control. PostgreSQL smoke must verify candidate debounce, partial preservation, grouped incident deduplication, recovery, cross-store isolation, and dashboard read-model redaction before this milestone is operationally closed. Real Merchant API calls remain a separate staging gate.

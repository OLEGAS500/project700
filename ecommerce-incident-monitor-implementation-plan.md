...
Milestone 7.4 Telegram HTTP transport is code-complete. The injectable transport sends one `sendMessage` request with the configured chat/topic destination and immutable HTML content, persists stable provider message IDs, distinguishes transient timeout/network/5xx/rate-limit failures from permanent provider failures, and honors a clamped Telegram `retry_after` override without weakening lease fencing. Operational sign-off requires the updated PostgreSQL smoke suite and the full clean GitHub CI pipeline to pass.

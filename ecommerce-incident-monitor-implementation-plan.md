...
Milestone 8.3.2 incident detail UI is complete and operationally verified. It adds a read-only server-rendered `/incidents/[id]` route backed directly by the dashboard detail facade, with UUID validation before database access; safe header facts, signals, bounded evidence samples, lifecycle timeline, read-only comments, and redacted delivery state; plus loading, invalid-link, not-found, and read-failure states. Incident list titles now link only to this real user-facing route. Clean GitHub CI passed the full validation pipeline, including the PostgreSQL smoke suite. It deliberately excludes incident write actions, polling, charts, settings, authentication, and scheduling.

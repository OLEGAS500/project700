...
Milestone 9.5 Merchant Center item-issue incident signals is complete and operationally verified through clean PostgreSQL CI. Complete successful item-issue checks are grouped by a configuration-scoped debounce candidate: the first affected snapshot creates a pending candidate, a second distinct complete affected snapshot confirms one `merchant_item_issues` incident, and a complete healthy snapshot dismisses an unconfirmed candidate. Partial, failed, authentication-failed, or configuration-incompatible checks do not create or dismiss business conclusions. The collector persists an immutable account-derived configuration hash in each item-issues source check, and the evaluator/recovery path take a store-scoped advisory lock, lock both the item-issues source-check row and store row, and read status plus items through the same transaction client; missing, malformed, or mismatched hashes are ignored without business transitions. The lock query uses an inner join so PostgreSQL can apply `FOR UPDATE` without locking the nullable side of an outer join. Severity is `critical` when any normalized issue has critical/error-like severity and `warning` otherwise; evidence is persisted as bounded issue samples and grouped signals without raw provider payloads. Recovery uses the shared transition primitive and requires two consecutive complete healthy checks; a resolved incident is not reopened by a later occurrence. Existing incident list/detail and alert-preferences UI expose the new type, labels, severity, issue evidence, and mute control, including all six supported incident types in the mute contract. Clean CI runs `29438862454` and `29438865378` passed 242 unit tests, 38/38 PostgreSQL smoke tests, typecheck, lint, and build, including reconnect isolation between Merchant accounts. Real Merchant API calls remain a separate staging gate.

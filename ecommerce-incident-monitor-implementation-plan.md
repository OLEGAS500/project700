...
Milestone 9.4.2 Merchant Center item-level issues is complete and operationally verified through clean PostgreSQL CI; the manual provider gate remains pending. The server-only worker uses the official `products/v1/accounts/{accountId}/products` endpoint, reads `productStatus.itemLevelIssues`, follows `nextPageToken` under bounded page, product, token-length, repeated-token, and total-deadline guards, and shares the existing fenced OAuth refresh path. Product identity is stable from `offerId` with a bounded product-name fallback; issue code, severity, resolution, attribute, reporting context, descriptions, documentation URLs, and country lists are normalized and length-limited, while duplicate issues are removed. A complete successful run persists an authoritative `merchantDataKind = product_identity` inventory for every normalized Merchant product and stores normalized issue arrays only for affected products. A partial or source-failed run cannot create an inventory conclusion: it preserves the prior complete inventory and conservatively upserts only collected issue observations. Unit fixtures cover the official endpoint, current response shape, pagination, bounded termination, deduplication, safe provider failures, and malformed responses; clean GitHub CI passed 239 unit tests and 34/34 PostgreSQL smoke tests, plus typecheck, lint, and build. Product import, remediation UI, incident candidates, and real Merchant API calls remain outside this slice.

Milestone 9.5 Merchant Center item-issue incident signals is complete and operationally verified through clean PostgreSQL CI. Complete successful item-issue checks are grouped by a configuration-scoped debounce candidate: the first affected snapshot creates a pending candidate, a second distinct complete affected snapshot confirms one `merchant_item_issues` incident, and a complete healthy snapshot dismisses an unconfirmed candidate. Partial, failed, authentication-failed, or configuration-incompatible checks do not create or dismiss business conclusions. The collector persists an immutable account-derived configuration hash in each item-issues source check. Persistence locks the snapshot and store, fences the first hash, uses the account-independent logical check key `merchant_center:item_issues`, and ignores conflicting or malformed later writes before touching shared item rows. The evaluator/recovery path follows the same snapshot/store lock order, then locks the single logical source-check row and reads status plus items through the same transaction client; missing, malformed, or mismatched hashes are ignored without business transitions. The lock query uses an inner join so PostgreSQL can apply `FOR UPDATE` without locking the nullable side of an outer join. Severity is `critical` when any normalized issue has critical/error-like severity and `warning` otherwise; evidence is persisted as bounded issue samples and grouped signals without raw provider payloads. Recovery uses the shared transition primitive and requires two consecutive complete healthy checks; a resolved incident is not reopened by a later occurrence. Existing incident list/detail and alert-preferences UI expose the new type, labels, severity, issue evidence, and mute control, including all six supported incident types in the mute contract. Clean CI runs `29440431729` and `29440434155` passed 242 unit tests, 38/38 PostgreSQL smoke tests, typecheck, lint, and build, including same-snapshot account A/B/A fencing, reconnect isolation, and concurrent partial-upsert serialization. Real Merchant API calls remain a separate staging gate.

Milestone 9.6 Merchant incident triage read model is complete and operationally verified through clean PostgreSQL CI. Merchant incident detail reads the persisted item-issues rows from the incident's immutable opened snapshot, groups normalized issue codes, counts affected products and issues, and prioritizes products by normalized severity (`critical`, `high`, `normal`). The dashboard exposes only bounded, whitelisted triage fields: issue code, priority, affected attributes, stable key, offer ID, title, and counts; provider descriptions, documentation URLs, raw payloads, and remediation write actions are excluded. The read model independently bounds products, issues per product, normalized issue budget, issue groups, group severities and attributes, and product issue codes and attributes. It reports `productsTruncated`, `issuesTruncated`, and `groupsTruncated` so SSR can explain every bounded layer. Malformed issue records are ignored, duplicate normalized issues are removed, and the product list is bounded. This slice is read-only and does not change incident lifecycle, provider calls, alert decisions, or Merchant Center persistence semantics. Exact-head GitHub CI runs `29442699401` and `29442702077` passed 246 unit tests, 38/38 PostgreSQL smoke tests, typecheck, lint, and build, including the adversarial bounded-input regression. Real Merchant API staging remains a separate provider gate.

Milestone 9.7 Merchant read-only remediation queue is functionally verified and merged. Merchant incident detail adds a store- and incident-scoped queue sourced only from the immutable `opened_snapshot_id`; later Merchant snapshots cannot enter the queue. The queue supports bounded keyset pagination, deterministic priority/issue-count/stable-key/title sorting, issue-code and normalized-severity filters, critical/high/normal priority filters, and bounded literal search across offer ID, stable key, and title. Group counters remain visible in the triage summary, issue-code links preserve active filters while opening the queue, and active filters plus pagination are represented in the URL. Cursor encoding and the web query boundary share a 4096-character limit, while sort keys and issue fields use PostgreSQL-compatible Unicode code-point truncation. The read model exposes only normalized issue codes, priorities, attributes, identifiers, titles, and counts; no provider calls, lifecycle changes, or remediation write actions are included. Exact-head GitHub CI runs `29446990020` and `29446993139` passed 251 unit tests, 41/41 PostgreSQL smoke tests, typecheck, lint, and build. Squash merge is `e1fe7fc`. Seeded visual QA and real Merchant API staging remain independent pending gates for full operational sign-off. The reproducible visual checklist is `docs/runbooks/merchant-remediation-queue-visual-qa.md`.

Milestone 9.8.1 Cross-source Offer ID mapping is complete and operationally verified through clean PostgreSQL CI. The read-only `getCrossSourceProductMatchSummary()` facade compares explicitly selected immutable feed and Merchant `product_identity` inventories for the same store under a repeatable-read transaction. It uses only a complete successful Merchant inventory with the supported version and an immutable account-derived configuration hash that still matches the connected store; missing, partial, failed, legacy issue-only, cross-store, or configuration-incompatible pairs cannot produce comparable mapping output. Matching is deliberately two-stage: unique normalized offer IDs win first, then only remaining records may match by a unique normalized stable key when one side has no offer ID. Different nonempty offer IDs never match through a shared stable key, while duplicate offer or stable identities remain `ambiguous`. The result separates `matched`, `feed_only`, `merchant_only`, and `ambiguous`, returns bounded whitelisted samples and capped totals, and never reads later snapshots or provider payloads. No incident mutations, provider calls, or remediation write actions are included. Merchant Product storage identity is the SHA-256 of a validated provider resource name, so different language/feed-label resources with one Offer ID are retained separately and map to `ambiguous` rather than being collapsed. A shared Offer ID normalizer trims, collapses whitespace, and lowercases values in feed collection, Merchant collection, source matching, TypeScript mapping, and the PostgreSQL CTE. PostgreSQL regressions cover full healthy inventories with zero issues, issue-bearing subsets, partial compatibility fences, one-sided-offer stable fallback, differing-offer rejection, duplicate stable fallback on either side, cross-store isolation, later-snapshot isolation, configuration fencing, provider resource multiplicity, independent issue arrays, and feed/Merchant whitespace equivalence through collection, persistence, and mapping. Exact-head CI run `29482615484` for `0f5da07` passed unit tests, PostgreSQL smoke, the fixture lifecycle smoke, typecheck, lint, and build. Real Merchant API staging remains a separate provider gate.

Milestone 9.8.2 Feed x Merchant correlation is complete and operationally verified through clean PostgreSQL CI. It augments one already-confirmed `catalog_drop` rather than creating a second incident or changing the shared recovery state machine. The mutation path is internal and candidate-bound: it locks the immutable catalog-drop candidate, requires an incident with the matching candidate link and captured baseline/configuration/threshold context, and reads only the candidate's immutable first drop snapshot. A unique candidate link on the incident and a partial unique event index allow one correlation event per incident, even when a later confirmation snapshot is replayed. The correlation requires a completed snapshot; successful feed, Merchant status, and complete Merchant identity checks; matching current account-derived hashes; a comparable mapping with no ambiguous identities or count-cap truncation; and reconciliation of `matched + feed_only` with the stored current feed count. Merchant-only identity evidence must itself have a nonzero matched inventory and satisfy the same frozen absolute and percentage thresholds as the catalog drop. Its bounded 7-14 observation Merchant-approved baseline uses the actual status-check `finished_at` as the cutoff and ordering key, selects at most one status observation per snapshot, and excludes observations completed after the candidate snapshot even if their snapshot rows were created earlier. Bounded samples remain valid evidence, while partial, failed, incomplete, legacy, configuration-mismatched, ambiguous, unreconciled, threshold-insufficient, or count-truncated evidence produces no lifecycle mutation. A successful correlation raises the existing catalog-drop confidence, stores bounded whitelisted samples and two signals, records a deduplicated timeline event, and runs before immutable open-alert payload creation. Standard catalog-drop recovery remains feed-based and continues to require two consecutive healthy feed checks, so missing or partial Merchant evidence cannot block recovery or reopen a resolved incident. The status collector captures the immutable Merchant account configuration hash with each aggregate-status result. PostgreSQL smoke covers account-fenced approved counts, partial/ambiguous/configuration rejection, threshold-sized identity evidence, feed-count reconciliation, forged context rejection, candidate-pinned replay, late status-check baseline exclusion, one observation per snapshot, concurrent partial replacement, and feed recovery without provider calls. Exact-head CI run `29492617221` for `55ccfa2` passed 260 unit tests, 53/53 PostgreSQL smoke tests, the fixture lifecycle smoke, typecheck, lint, and build. Real Merchant API staging remains a separate provider gate.

Milestone 10.1 adds a reproducible local PostgreSQL v0.1 demo acceptance fixture and runbook. The fixture seeds seven Feed baseline observations at 642 with Merchant approved counts at 620, then uses the existing production contracts to persist a 17-category / 21-Feed / 30-Merchant drop, confirm the catalog-drop candidate, and let candidate-bound Feed × Merchant correlation create the existing incident, signals, event, and immutable open-alert payload. Its current Merchant identity inventory is complete and reconciles 21 normalized Offer-ID matches, zero Feed-only, 599 Merchant-only, and zero ambiguous records. A distinct fixture store demonstrates two safe Feed `source_unavailable` HTTP 503 checks after a healthy observation, producing source health without a product-loss candidate. Feed-only recovery commands then advance the same catalog-drop incident through recovering and resolved; all commands assert their lifecycle invariants, support replay, and clean only their named `.example.test` stores after alert dependencies. The runbook is `docs/runbooks/v01-demo-acceptance.md`. This local deterministic acceptance fixture does not call a provider and does not replace the separate real Merchant API staging gate or operational sign-off.

Milestone 10.2 staging readiness adds a public, uncached `GET /api/health` endpoint for Railway's healthcheck contract. It uses the existing Node.js database pool only to execute `SELECT 1`; it returns exactly `200 {"status":"ok"}` on success or `503 {"status":"unavailable"}` on any unavailable configuration or database path, without returning diagnostics, connection metadata, store data, provider data, or timestamps. The endpoint is explicitly dynamic and Node-runtime-only so it cannot be served from a static cache. Unit regressions cover both outcomes and ensure failure details are never exposed. `docs/runbooks/staging-readiness.md` describes the post-deployment Railway healthcheck configuration and safe verification. This slice does not add a scheduler, change incident decisions, connect a provider, or replace the v0.1 fixture or real Merchant API staging gate.

...
Milestone 8.3.2 incident detail UI is code-complete. It adds a read-only server-rendered `/incidents/[id]` route backed directly by the dashboard detail facade, with UUID validation before database access; a true Next.js 404 for unknown valid incident IDs; safe header facts, signals, bounded evidence samples, lifecycle timeline, read-only comments, and redacted delivery state; plus loading, invalid-link, not-found, and read-failure states. Incident list titles now link only to this real user-facing route. PostgreSQL operational verification is pending the clean CI run for the final head commit. It deliberately excludes incident write actions, polling, charts, settings, authentication, and scheduling.
